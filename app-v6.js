...
async function optimizeImageFile(file) {
  ...
  if (!String(file.type || "").startsWith("image/")) throw new Error("image-type");
  ...
  for (const quality of [0.82, 0.7, 0.58, 0.46]) {
    ...
    if (blob && (blob.size <= MAX_STORED_PHOTO_BYTES || quality === 0.46)) break;
  }
...
async function publishAdventure(event) {
  ...
  try {
    ...
    const message = ["image-too-large", "image-too-large-after-optimization"].includes(error?.message)
      ? "That photo is too large after compression. Try a smaller image."
